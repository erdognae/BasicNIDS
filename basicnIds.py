...
if not os.path.exists('scrapy_iplist.txt'): # Zararlı ip adreslerinin bulunduğu dosya. Bu dosya dinamik olarak web scraping  ile sürekli güncellenecek şekilde ayarlanabilir.     print("Error: 'scrapy_iplist.txt' file not found.")
    exit()
...
try:
    host = socket.gethostbyname(socket.gethostname()) #Bu kod, bilgisayarın ağ paketlerini doğrudan dinlemeye ve manipüle etmeye yarayan bir ham soket oluşturur. 
...
while True:
    ...
    src_addr = socket.inet_ntoa(iph[8])  # socket.inet_ntoa() fonksiyonu, binary (ikili) formatta bir IP adresini, insan tarafından okunabilir bir IPv4 adresi formatına dönüştüren bir Python fonksiyonudur. 4s listedeki 8 ve 9. elemanlara denk gelir.
